...
from tmatrix.paths import AGG_DATA, BENCHMARK_2X2
...
def sweep(spec, pol, out_dir):
    ...
    cmd = ([sys.executable, "-m", "tmatrix.aggregation.run_supercell",
            "--cell", "16", "--lmax", LMAX,
            "--pol", str(pol[0]), str(pol[1]), "--out", out_dir]
           + site_args(spec))
    r = subprocess.run(cmd, cwd=AGG_DATA, capture_output=True, text=True)
    if r.returncode != 0:
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jones_",
        ...
        for key, (spec, xdir) in CELLS.items():
            xp = os.path.join(AGG_DATA, xdir, "periodic_results.npz")
