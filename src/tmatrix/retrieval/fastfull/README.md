# `tmatrix.retrieval.fastfull` — fast full T-matrix from a coded Floquet cell
...
`tmatrix.aggregation` repo-vs-treams discrepancy (~0.1 in S, opposite passivity
...
retrieval in `src/tmatrix/retrieval/*.py` (its data and reports live under
`retrieval/`). That pipeline is complete and its conventions
...
| `m1_study.py` | the M1 deliverable: runs the whole comparison and writes `retrieval/results/fastfull/M1_DESIGN_STUDY.md` |
| `gate_a_run.py` | the M2 deliverable: blind recovery over the candidate set with a held-out encoding, writes `retrieval/results/fastfull/GATE_A_STUDY.md` |
...
```bash
python tests/retrieval/test_fastfull_core.py      # 27 gates
```

```bash
python tests/retrieval/test_fastfull_design.py    # 23 gates
```

```bash
python tests/retrieval/test_fastfull_ewald.py     # 14 gates (M2 coupling)
```

```bash
python tests/retrieval/test_fastfull_synthetic.py # 31 gates (M2 Gate A + provenance)
```

```bash
python -m tmatrix.retrieval.fastfull.m1_study --samples 700
```

```bash
python -m tmatrix.retrieval.fastfull.gate_a_run --trials 3
```

```bash
python -m tmatrix.retrieval.fastfull.opt_marginalized --samples 300
```

```bash
python -m tmatrix.retrieval.fastfull.design --evaluate 26.0,33.8,90,0,0.090,-0.460 --lam 20 --benchmark --kz-min 0 --wood-margin 0 --orders 1,64 --area-max 5000
```
...
* **The error level is a systematic discrepancy, not a noise floor.** σ comes
  ...
  dominated by model error (`retrieval/results/REAL_RETRIEVAL.md` §4.3), so it
  does not average down over the hundreds of modal entries a coded cell
  provides.
