...
Designs are read from `retrieval/results/fastfull/m1_study.json` so the study
and the recovery cannot drift apart.  The reference wheel T is used only as
the synthetic ground truth to be recovered blind, and the CONTROL run uses an
exactly-D4h passive draw so that the reference file's own symmetry violation
is separated from the recovery error.

Run:  python -m tmatrix.retrieval.fastfull.gate_a_run [--trials 3] [--lam 8]
"""
import argparse
import json
import os
...
def _markdown(rec):
    L = ["# Gate A — blind noisy synthetic recovery (M2)", "",
         "Generated by `tmatrix.retrieval.fastfull.gate_a_run`.  Blind "
         "recovery of "
