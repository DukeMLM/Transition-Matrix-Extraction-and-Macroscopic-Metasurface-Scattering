...
#: this package's own source directory -- the provenance hash covers the code
#: that produced a run, so this is deliberately a source path, not a data one
...
from tmatrix.paths import FASTFULL_RESULTS, RETRIEVAL_RESULTS, SRC
...
SNAPSHOT_FILES = [
    ...
    os.path.join(SRC, "aggregation", "vswf.py"),
    os.path.join(SRC, "aggregation", "tmat_io.py"),
    os.path.join(SRC, "aggregation", "translate.py"),
    ms.REF_TMAT,
    os.path.join(RETRIEVAL_RESULTS, "fit_sigma_from_closure.npz"),
]
