...
Run:  python -m tmatrix.retrieval.fastfull.m1_study [--quick] [--samples N]
                                                    [--out DIR]
"""
import argparse
import json
...
from tmatrix.paths import BENCHMARK_SINGLE, FASTFULL_RESULTS

REF_TMAT = os.path.join(BENCHMARK_SINGLE, "saw_gold_wl15p0025um.tmat.h5")
OUT_DIR = str(FASTFULL_RESULTS)
...
def verify_with_coupling(design, k, modes, B, T_ref, n_ensemble=6,
    ...
    result.  Now that `tmatrix.retrieval.fastfull.ewald` supplies a converged
    C for these cells, the caveat can be discharged directly: this recomputes
    the wheel metrics over a passive D4h ensemble WITH C, and against the
    reference T, and reports how far each number moved.
...
def run(samples=800, polish=3, quick=False, out_dir=OUT_DIR, seed=20260807):
    ...
    out = dict(generated_by="tmatrix.retrieval.fastfull.m1_study",
...
def write_markdown(out, path):
    L = ["# M1 design study - rank, SNR and cost of a coded Floquet cell", "",
         "Generated by `tmatrix.retrieval.fastfull.m1_study`.  "
         "Milestone M1 of "
         ...
         "(`retrieval/results/fit_sigma_from_closure.npz`): %.4e at %.1f um, "
         "%.4e at "
         ...
         "discrepancy, not iid noise (`retrieval/results/REAL_RETRIEVAL.md` "
         "par. 4.3), "
