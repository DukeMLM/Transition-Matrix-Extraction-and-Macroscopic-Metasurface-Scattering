...
from tmatrix.aggregation.vswf import ELECTRIC, sphere_quadrature, vswf_fields  # noqa: E402
...
def projector_matrices(modes, Ds=None):
    ...
    imag_resid = max(maxabs(P_c4.imag), maxabs(P_grp.imag))
    P_c4 = P_c4.real
    P_grp = P_grp.real
...
def build_c4v_reciprocity_basis(modes, verify_numeric=True, k=1.0,
    ...
    if verify_numeric:
        D_num, numeric = derive_sigma_v_numeric(modes, k=k,
                                                r_sample=r_sample)
        err_num = maxabs(D_num - D_sigma)
        sq_err_num = maxabs(D_num @ D_num - np.eye(n))
    ...
    ortho_resid = maxabs(G - np.eye(rank_full))

    # -- consistency: eigh basis vs the operator form of P
    perm, sign = reciprocity_perm_sign(modes)
    inv_resid = max(
        maxabs(apply_full_projector(Bk, Ds, perm, sign) - Bk) for Bk in B)
    ...
    basis_proj_resid = maxabs(PT_basis - PT_apply)
    comm_T_resid = maxabs(
