...
from tmatrix.aggregation.vswf import ELECTRIC, far_field_amplitude         # noqa: E402
from tmatrix.retrieval.sparams_oblique import (khat_from_angles, pol_basis,
...
def fit_frequency(fm, ifreq, B, S_meas, angle_indices=None, weights=None,
    ...
    **lsq_kwargs                   forwarded to scipy least_squares
    ...
    scale = max(1.0, maxabs(T0_hat))
    proj_resid = maxabs(
        par.apply_full_projector(T0_hat, Ds, perm, sign) - T0_hat)
    rec_resid = maxabs(par.apply_rec(T0_hat, perm, sign) - T0_hat)
