...
from fit import DIRECTION_DEFAULT, resolve_angles, entry_label  # noqa: E402
...
def physical_checks(T0_stack, modes=None, perm=None, sign=None,
    ...
    if perm is None:
        ...
        rec = np.array([_maxabs(par.apply_rec(T, perm, sign) - T)
                        for T in T0_stack])
...
def _cd_noise_study(fm, band, angle, hyp, sigmas, n_trials, seed=4242,
    ...
    for h in deembed.label_hypotheses(extended=(family == "extended")):
        ...
        sep = min(sep, max(_maxabs(a - S11p), _maxabs(b - S21p)))
    out = {}
    for sg in sigmas:
...
def _channel_dict_selftest(fm, band, tol_strict=2e-3, n_trials=20,
    ...
    for h in hyps:
        ...
        inv_err = max(inv_err, _maxabs(a2 - A), _maxabs(b2 - Bb))
    ...
    for h in ext_hyps:
        ...
        rt_err = max(rt_err, _maxabs(a2 - A), _maxabs(b2 - Bb))
        a3, b3 = deembed.apply_hypothesis(a1, b1, h)
        if max(_maxabs(a3 - A), _maxabs(b3 - Bb)) > 0:
            n_noninv += 1
    ...
    for ia in fitmod.ANGLE_SETS["campaign"]:
        s11, s21 = reference_blocks(fm, band, fm.theta_deg[ia],
                                    fm.phi_deg[ia])
        xp = max(_maxabs(s11[[0, 1], [1, 0]]), _maxabs(s21[[0, 1], [1, 0]]))
        sep, dmin = np.inf, np.inf
        for h in ext_hyps:
            if h == ext_hyps[0]:
                continue
            a, b = deembed.apply_hypothesis(s11, s21, h)
            sep = min(sep, max(_maxabs(a - s11), _maxabs(b - s21)))
...
def selftest(argv=None):
    ...
    dT_ctrl = max(_maxabs(ctrl["T0"][i] - T_bp[i]) for i in ctrl["ifreqs"]) \
    ...
    dS_plumb = max(_maxabs(rows_for_angles(S_round[i], all_ang)
