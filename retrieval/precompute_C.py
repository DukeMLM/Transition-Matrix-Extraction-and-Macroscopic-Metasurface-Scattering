...
from translate import make_quad, square_lattice_shells, translation_shells

DATA = os.path.join(HERE, "..", "test", "single",
                    "saw_gold_wl15p0025um.tmat.h5")
OUT_DEFAULT = os.path.join(HERE, "results", "C_bloch")
CONSOLIDATED = os.path.join(HERE, "results", "C_bloch.npz")
...
def richardson(C_tapers, Rcs):
    """Lagrange extrapolation in 1/Rc^2 -> 0, exactly as
    translate.assemble_shell_sum / bloch_lattice.assemble_shell_sum_bloch."""
    x = 1.0 / np.asarray(Rcs, dtype=float) ** 2
    C = np.zeros_like(C_tapers[0])
