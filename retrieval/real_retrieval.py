...
import precompute_C as pc                              # noqa: E402
...
from validate_against_reference import gate            # noqa: E402

# --------------------------------------------------------------- locations
RUNS_DIR = os.path.join(HERE, "cst_runs")
...
def assembly_checks(fm, ds, sigma, make_figures=True, tag="real"):
    ...
    j0 = ang.index(0)
    d11 = _maxabs(S11c[j0, 0, 0] - S11c[j0, 1, 1])
    d21 = _maxabs(S21c[j0, 0, 0] - S21c[j0, 1, 1])
    ...
    for ia in PHI225_ANGLES:
        j = ang.index(ia)
        off[ia] = max(_maxabs(S11c[j, 0, 1]), _maxabs(S11c[j, 1, 0]),
                      _maxabs(S21c[j, 0, 1]), _maxabs(S21c[j, 1, 0]))
    ...
    for ia in PHI225_ANGLES:
        ...
        for nm, Sm in (("S11", S11j[j]), ("S21", S21j[j])):
            a, b = Sm[0, 1], Sm[1, 0]
            scale = max(_maxabs(a), _maxabs(b), 1e-300)
            d[nm] = dict(minus=_maxabs(a - b) / scale,
                         plus=_maxabs(a + b) / scale, scale=scale)
        rec[ia] = d
...
def noise_calibration(fm, runs_dir=RUNS_DIR, manifest=None, tag="real",
    ...
    for th in thetas:
        ...
        for a in (1, 2):
            ...
            rec["mode%d" % a] = dict(
                dS21=_maxabs(s21 - ana), mag_dev=chk["mag_dev"],
        ...
        for a, b in ((1, 2), (2, 1)):
            for side in ("Zmin", "Zmax"):
                xmax = max(xmax, _maxabs(blocks[_entry(side, a, b)][1]))
        rec["crosspol_max"] = xmax
        rec["copol_degeneracy"] = _maxabs(
    ...
    ent = sorted(base)
    d_raw = max(_maxabs(pert[e][1] - base[e][1]) for e in ent)
    d_copol = max(_maxabs(pert[_entry("Zmin", a, a)][1]
    ...
    d_de = max(_maxabs(S11_p - S11_b), _maxabs(S21_p - S21_b))
...
def continuation_study(fm, B, S_by_freq, ifreq_list, fit_angles,
    ...
    if born is not None:
        ...
        dT = np.array([_maxabs(T_cont[i] - born["T0"][i]) for i in common])
...
def write_report(path, ctx):
    ...
    if cont is None:
        ...
        A("`max |T0_cont - T0_born|` over the band = **%.3e** against a "
          ...
          % (cont.get("dT_max", np.nan), _maxabs(fm.data.T),
             "on essentially the same solution"
             if cont.get("dT_max", 1) < 1e-6 * _maxabs(fm.data.T)
...
def main(argv=None):
    ...
    T_p68 = np.array([par.unpack(par.pack(fm.data.T[i], B68), B68)
                      for i in range(fm.nf)])
    span_id = _maxabs((T_bp - T_p68)[:, mask])
    ...
    chk = max(_maxabs(S_by_freq[i][ia, 0] - ds["S11_jones"][j][..., i])
