...
from vswf import ELECTRIC, MAGNETIC, sphere_quadrature, vswf_fields
...
def build_d4h_reciprocity_basis(modes, verify_numeric=True, k=1.0,
    ...
    I = np.eye(n)
    sh_sq_resid = _maxabs(D_sh @ D_sh - I)
    commute_resid = _maxabs(D_sh @ D_sv - D_sv @ D_sh)
    ...
    ortho_resid = _maxabs(Bf @ Bf.T - np.eye(rank_full))
    inv_resid = max(_maxabs(apply_d4h_projector(Bk, Ds, perm, sign) - Bk)
                    for Bk in B)
    ...
    even = hsigns > 0
    odd = ~even
    parity_leak = max(_maxabs(B[:, even][:, :, odd]),
                      _maxabs(B[:, odd][:, :, even]))
    ...
    basis_proj_resid = _maxabs(pz.unpack(pz.pack(PT_apply, B), B) - PT_apply)
