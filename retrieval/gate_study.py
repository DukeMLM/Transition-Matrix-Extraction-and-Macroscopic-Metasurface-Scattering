...
def orbit_pure_basis(B68, mask, modes, tol=1e-10):
    ...
    gram = _maxabs(flat @ flat.T - np.eye(r))
...
def build_spans(fm, ifreq, B68, eng68, T_proj, angles, direction, sigma,
    ...
    def add(name, kind, B, mask, thr, extra):
        ...
        dS = S_proj - S_span
        if mask is not None:
            num = _maxabs(T_span[mask] - T_proj[mask])
            den = max(_maxabs(T_proj[mask]), 1e-300)
            ident = num / den
        else:
            m25 = spans[0]["mask"] if spans else None
            ident = (_maxabs(T_span[m25] - T_proj[m25])
                     / max(_maxabs(T_proj[m25]), 1e-300)
        ...
        d = dict(name=name, kind=kind, threshold=thr, B=B, mask=mask,
                 ...
                 ident_rel=float(ident), floor_max=_maxabs(dS),
...
def leverage_table(fm, ifreq, angles, T_proj, direction, spans,
    ...
    supp = conf & (np.abs(T_proj) > 1e-14 * _maxabs(T_proj))
...
def run_frequency(fm, ifreq, B68, args, store):
    ...
    dab = _maxabs(ra["T0_hat"] - rb["T0_hat"])
    scale = max(_maxabs(ra["T0_hat"]), _maxabs(rb["T0_hat"]))
    ...
    model_err_T = _maxabs(T_proj - par.unpack(t_true_g, Bg))
