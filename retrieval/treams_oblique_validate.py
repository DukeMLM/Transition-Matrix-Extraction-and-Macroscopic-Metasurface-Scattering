...
from sparams_oblique import pol_basis
import precompute_C as pc

TMAT_FILE = r"D:/Claude/T matrix/test/single/saw_gold_wl15p0025um.tmat.h5"
OUT_FILE = os.path.join(HERE, "results", "treams_oblique_check.npz")
SIGN_FILE = os.path.join(HERE, "results", "treams_kpar_sign.npz")
PITCH_UM = 2.0

# ----------------------------------------------------------------------------
# Windows / numpy>=2 compatibility patches, copied VERBATIM from
# aggregation/treams_reference.py (load-bearing; casting only, no physics)
# ----------------------------------------------------------------------------
...
def our_jones(fm, ifreq, theta_deg, phi_deg, cache_idx):
    ...
    """
    from sparams_oblique import sparams_oblique
    T = fm.data.T[ifreq]
...
def sweep(fm, tms, sign, stride, val_angles=None, out_file=OUT_FILE):
    ...
    if ja0 is None:
        ...
        stored_t = np.load(os.path.join(HERE, "..", "aggregation", "results",
                                        "treams_reference.npz"))
        stored_p = np.load(os.path.join(HERE, "..", "aggregation", "results",
