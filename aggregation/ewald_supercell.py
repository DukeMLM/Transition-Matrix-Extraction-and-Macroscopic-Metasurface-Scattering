...
# treams calls its gufuncs with `where=` and no `out=`; numpy 2 warns that the
# masked-out entries are uninitialized.  treams fills them itself, and the
# warning fires from inside the cast wrapper below rather than from treams, so
# it is filtered here rather than left to bury the run log.
warnings.filterwarnings(
    "ignore", message=".*'where' used without 'out'.*",
    category=UserWarning)

_PATCH_FLAG = "_supercell_cast_wrapped"


def _cast_wrap(uf):
    ins = uf.types[-1].split("->")[0]
...
def _apply_patch():
    """Windows / numpy>=2: the compiled gufuncs declare integer inputs as C
    long (int32) while numpy 2.x defaults to int64.  Casting only."""
    for mod in (sw, pw, cw, _tl):
        for name in dir(mod):
            uf = getattr(mod, name)
            if (isinstance(uf, np.ufunc)
                    and any("l" in s.split("->")[0] for s in uf.types)):
